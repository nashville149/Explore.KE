...
export default function Login() {
  ...
  const [generatedOtp, setGeneratedOtp] = useState(""); // Store the OTP sent to user
  const [sendingOtp, setSendingOtp] = useState(false); // Track OTP sending status
  ...
  // 3. Create an email template with {{otp}} and {{email}} variables
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      setMessage({ 
        type: "success", 
        text: `OTP has been sent to ${email}. Please check your inbox.` 
      });
    } catch (error) {
      console.error("Failed to send OTP:", error);
      
      // Fallback: Still show OTP step but show error message
      setMessage({ 
        type: "error", 
        text: "Failed to send email. Please check your EmailJS configuration or try again." 
      });
      setOtpStep(true);
    } finally {
      setSendingOtp(false);
  ...
  // Handles the initial form submission (email/password)
        ...
        {!otpStep ? (
          ...
          >
            ...
            <button
              ...
              type="submit"
              disabled={!email.trim() || !password.trim() || sendingOtp}
              aria-disabled={!email.trim() || !password.trim() || sendingOtp}
            >
              {sendingOtp ? "Sending OTP..." : mode === "login" ? "Sign In" : "Create Account"}
            </button>
          </form>
        ) : (
          <form
          ...
          >
            <div style={{ textAlign: "center", marginBottom: "20px" }}>
              <p style={{ color: "#FFB347", fontWeight: "600", fontSize: "14px", marginBottom: "8px" }}>
                OTP sent to {email}
              </p>
            ...
            <div className="input-group">
              ...
              />
              <div
                className={`radio-circle ${otp.length === 6 ? "checked" : ""}`}
                aria-label={otp.length === 6 ? "Selected" : "Not selected"}
                role="checkbox"
                aria-checked={otp.length === 6 ? "true" : "false"}
              />
            </div>
